...
sub string {
	...
	$string = $self->{lop}->string($bop->{precedence}) . $bop->{string} . $self->{rop}->string($bop->{precedence});
...
sub demote {
	...
	my $context = $self->context;
	my $other   = shift;
	$other = $self unless defined $other;
...
sub apply {
	...
	my $method  = shift;
	my $other   = shift;
	$context->Package("Inequality")->new($context, $self->demote->$method($self->demote($other), @_), $self->{varName});
}
...
sub string {
	...
	if ($self->{leftInfinite}) {
		...
		return $a->string . ($open eq '(' ? ' < ' : ' <= ') . $x . ($close eq ')' ? ' < ' : ' <= ') . $b->string;
	}
}
...
sub TeX {
	...
	if ($self->{leftInfinite}) {
		...
		return $a->TeX . ($open eq '(' ? ' < ' : ' \le ') . $x . ($close eq ')' ? ' < ' : ' \le ') . $b->TeX;
	}
}
	...
	my ($and, $or, $le, $ge, $ne, $open, $close) = @{
		{
			...
			TeX    => [
				'\hbox{ and }', $op->{TeX} || $op->{string} || '\hbox{ or }',
				' \le ', ' \ge ', ' \ne ', '\left(', '\right)'
			],
		}->{$method}
	};
