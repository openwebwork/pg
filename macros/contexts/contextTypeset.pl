...
sub _check {
	...
	if (scalar(@{ $self->{params} }) == 1 && $self->{params}[0]->type eq 'Matrix') {
		...
		my $c = 0;
		foreach my $r (@rows) { $c = $r->length if $r->length > $c }
...
sub TeX {
	...
	$min = (defined($min) ? $x->TeX . ($fn->{allowArrow} ? '\to ' : '=') . $min : $x->TeX)
		if defined $x && !$fn->{dx};
	my $TeX = $fn->{TeX} . (defined($min) ? "_{$min}" : "") . (defined($max) ? "^{$max}" : "") . " " . $self->{op}->TeX;
	$TeX .= '\,d' . $x->TeX if defined $x && $fn->{dx};
	if   (defined($precedence) and $precedence > $fn->{precedence}) { $TeX = '\left(' . $TeX . '\right)' }
	else                                                            { $TeX = "{$TeX}" }
	return $TeX;
}
...
sub makeList {
	...
	$rop =
		...
		if $rop->{def}{isComma};
	return (($self->{lop}{isSemicolon} ? $lop->makeList : $lop), ($self->{rop}{isSemicolon} ? $rop->makeList : $rop));
}
...
sub TeX {
	...
	my $uop = $self->{def};
	$TeX = (defined($uop->{TeX}) ? $uop->{TeX} : $uop->{string});
	if   ($uop->{associativity} eq "right") { $TeX = $self->{op}->TeX($uop->{precedence}) . $TeX }
	else                                    { $TeX = $TeX . $self->{op}->TeX($uop->{precedence}) }
	return $TeX;
}
...
sub new {
	...
	if ($paren && $close && $paren->{formInterval}) {
		$paren = $parens->{interval}
			if (
				$paren->{close} ne $close
				|| (scalar(@{$coords}) == 2
					&& ($coords->[0]->{isInfinite} || $coords->[1]->{isInfinite}))
			);
	}
	my $type = Value::Type(
		$paren->{type}, scalar(@{$coords}), $entryType,
		list       => 1,
		formMatrix => $paren->{formMatrix}
	);
	...
	$open  = '' if $open eq 'start';
	$close = '' if $close eq 'start';
	my $list = bless {
		coords     => $coords,
		type       => $type,
		open       => $open,
...
$context{Typeset}->parens->add('<[' => { type => "Vector", close => ']>' },);
