...
sub add_histogram {
	...
	foreach my $dataSet (@accumulatedDataSets) {
		...
		foreach my $count (@frequencies) {
			...
			$graphRef->lineTo($binBoundaries[$lupe], $currentPlot + 0.25 + $multiplier * $count / $totalDataPoints,
				$black, 2);
			$graphRef->lineTo(
				$binBoundaries[ $lupe + 1 ],
				$currentPlot + 0.25 + $multiplier * $count / $totalDataPoints,
				$black, 2
			);
		...
		$graphRef->lineTo(
			$extrema[0] - $width * 1.1,
			$currentPlot + 0.25 + $multiplier * $maxFrequency / $totalDataPoints,
			$red, 1
		);

		# Add ticks for the frequency marks
		for ($lupe = 0; $lupe <= $maxFrequency; ++$lupe) {
			$graphRef->moveTo($extrema[0] - $width * 1.1, $currentPlot + 0.25 + $multiplier * $lupe / $totalDataPoints);
			$graphRef->lineTo(
				$extrema[0] - $width * 1.3,
