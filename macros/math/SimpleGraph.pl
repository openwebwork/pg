...
arrays of numbers, a reference to a MathObect C<Matrix>, a reference to an array
of arrays containing two strings and possibly a number (an edge set with
...
arrays of numbers or a reference to a MathObect C<Matrix>), then that matrix
...
Whether vertex labels that are added to the context are case sensitve or not.
...
sensitve.  So "a" will be accepted as correct for the vertex labeled "A".
