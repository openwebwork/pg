...
sub compare_basis {
	...
	if (defined($errors)) {
		...
		if (abs($ch_coord_mat->det_LR()) > $options{zeroLevelTol}) {
			...
			if (
				abs(
					Matrix->new_from_col_vecs(\@{ $rh_ans->{ra_student_ans} }) -
...
sub is_diagonal {
	...
	if (ref($matrix) eq 'ARRAY') {
		...
		if (ref($matrix[0]) ne 'ARRAY' or scalar(@matrix) != scalar(@{ $matrix[0] })) {
			warn
				"It is impossible for a non-square matrix to be diagonal, if you are a student, please tell your professor that there is a problem.";
		}
