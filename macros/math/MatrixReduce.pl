...
After selecting the Fraction context using C<Context('Fraction')->parens->set("[" => {formMatrix => 1})>,
C<$A = apply_fraction_to_matrix_entries($A);> applies C<Fraction()> to all of the entries of $A, which
makes subsequent matrix algebra computations with $A use fraction arithmetic.
...
=item Construct an n x n elementary matrix that will multiply row i by s:

C<$E = elem_matrix_row_mult(5,2,4);> creates a 5 x 5 identity matrix and swaps puts 4 in the
second spot on the diagonal.


=item Construct an n x n elementary matrix that will multiply row i by s:

C<$E = elem_matrix_row_mult(5,2,4);> creates a 5 x 5 identity matrix and puts 4 in the second spot on the diagonal.

=item Construct an n x n elementary matrix that will add s times row j to row i:

C<$E3 = elem_matrix_row_add(5,3,1,35);> creates a 5 x 5 identity matrix and puts 35 in the (3,1) position.


=item Perform the row switch transform that swaps (row i) with (row j):

C<$Areduced = row_switch($A,2,4);> swaps rows 2 and 4 in matrix $A.

=item Perform the row multiplication transform s * (row i) placed into (row i):

C<$Areduced = row_mult(A,2,10);> multiplies every entry in row 2 of $A by 10.

=item Perform the row addition transform (row i) + s * (row j) placed into (row i):

C<$Areduced = row_add($A,2,1,10);> adds 10 times row 1 to row 2 and places the result in row 2.
(Same as constructing $E to be the identity with 10 placed in entry (2,1), then multiplying $E * $A.)
