...
sub tableauEquivalence {
	return sub {
		...
		my $score = List(@rows1)->cmp(
			checker => sub {
				my ($listcorrect, $liststudent, $listansHash, $nth, $value) = @_;
				my $listscore = Vector($listcorrect)->cmp(parallel => 1)->evaluate(Vector($liststudent))->{score};
				return $listscore;
			}
...
sub lop_display {
	...
	$alignment = ($options{align}) ? $options{align} : ($tableau->align) ? $tableau->align : "|ccccc|cc|c|c|";
	@toplevel  = ();
...
sub new {
	...
	my $tableau = Class::Accessor->new({
		...
		current_b                 => undef,
		obj_col_index             =>
			undef,    # an array reference indicating the columns (e.g 1 or n+m+1) for the objective value or values
		toplevel           => undef,
		align              => undef,
...
sub assemble_tableau {
	...
	unless ($constraint_rows == $m and $constraint_cols == 1) {
		Value::Error(
			"constraint matrix b is $constraint_rows by $constraint_cols but should
...
sub current_tableau {
	...
	return _Matrix(@{ $self->current_constraint_matrix->extract_rows }, $self->current_objective_coeffs);
}
...
sub find_next_basis {
	my $self = shift;
	Value::Error("call find_next_basis as a Tableau method") unless ref($self) =~ /Tableau/;
	my $max_or_min     = shift;
	my $obj_row_number = shift // 1;
	my ($row_index, $col_index, $optimum, $unbounded) = $self->find_next_pivot($max_or_min, $obj_row_number);
	my $flag = undef;
	my $basis;
	if ($optimum or $unbounded) {
		...
		Value::Error("At least part of the pivot index (row,col) is not defined")
			unless defined($row_index)
			and defined($col_index);
...
# 	main::DEBUG_MESSAGE(" \@obj_row ",  join(' ', @obj_row ) );
	for (my $k = 1; $k <= $obj_col_dim; $k++) {
		...
		if (($obj_row_matrix->element($k) < -$zeroLevelTol and $max_or_min eq 'max')
			or ($obj_row_matrix->element($k) > $zeroLevelTol and $max_or_min eq 'min'))
		{
...
sub next_short_cut_basis {
	...
	my ($row_index, $col_index, $feasible_point, $infeasible_lop) = $self->next_short_cut_pivot();
	my $basis;
	$flag = undef;
