...
sub display_matrix {
	...
	my $styleParams = defined($main::defaultDisplayMatrixStyle) ? $main::defaultDisplayMatrixStyle : "(s)";
	...
	if ($main::displayMode eq 'TeX' or $opts{'force_tex'}) {
		for $j (@top_labels) {
			if ($main::displayMode ne 'HTML_MathJax') {
				$out .= '\smash{\raisebox{2.9ex}{\ensuremath{' . $j . '}}} &';
			} else {
...
#	$element = 0 if abs($element) < $main::functZeroLevelTolDefault;
				...
				$out .= '<table border="1"><tr><td>'
					if ($colcount == $opts{'box'}->[1] and $opts{'cnt'} == $opts{'box'}->[0]);
				$out .= $element;
...
sub proj_vec {
	...
	warn "Second input must be a matrix"
		unless ref($matrix) eq 'Matrix' and ${ $matrix->dim() }[1] == ${ $vec->dim() }[0];
