...
sub list_check {
	my $ans          = shift;
	my %params       = @_;
	my $value        = $params{entry_type};
	my $ltype        = $params{list_type};
	my $cmp          = $params{cmp};
	my $cmp_ans      = $params{cmp_ans};
	my @cmp_params   = (@{ $params{cmp_defaults} }, @{ $params{cmp_params} });
	my $split        = $params{split};
	my @split_params = (@{ $params{split_defaults} }, @{ $params{split_params} });
	my @cmp          = @{ $ans->{elements} };
	...
	my $showHints       = defined($params{showHints})       ? $params{showHints}       : $showPartialCorrectAnswers;
	my $showLengthHints = defined($params{showLengthHints}) ? $params{showLengthHints} : $showPartialCorrectAnswers;
	my $partialCredit   = defined($params{partialCredit})   ? $params{partialCredit}   : $showPartialCorrectAnswers;
