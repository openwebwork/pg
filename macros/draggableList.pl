...
sub _draggableList_init {
  PG_restricted_eval("sub DraggableList {new draggableList(\@_)}");

  main::POST_HEADER_TEXT(main::MODES(TeX=>"", HTML=><<'  END_SCRIPTS'));
    <script type="text/javascript" src=" https://ajax.googleapis.com/ajax/libs/jquery/1.7.2/jquery.min.js"></script>
    <script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/jqueryui/1.8.18/jquery-ui.min.js"></script>
    <script type="text/javascript" src="/webwork2_course_files/301/js/jquery.ui.touch-punch.min.js"></script>
    <script>
    function _draggableUpdate () {
...
package draggableList;

my $n = 0;  # number of draggable lists so far
  ...
  main::POST_HEADER_TEXT(main::MODES(TeX=>"", HTML=><<"  SCRIPT_AND_STYLE"));
    <script type="text/javascript">
      \$(document).ready(function() {
        \$(".draggable-$id").sortable({
          connectWith: ".draggable-$id",
          update: _draggableUpdate
        });
      });
    </script>
    <style type="text/css">
      .draggable-$id-container {
        color: #000000; /*#388E8E;*/
        width: 300px;
        float: left;
...
sub AnswerRule {
  ...
  main::TEXT('<div style="display:none" id="Proof">'.$rule.$ext.'</div>');
}
...
sub Bucket {
  ...
  my @lines = (
    ...
    '<div class="draggable-'.$id.'-label">'.$label.'</div>',
    '<ul class="draggable-'.$id.'" id="'.$id.'-'.$name.'" data-id="'.$id.'" data-ans="'.$ans.'">'
  );
