...
sub _check {
	...
	my @argTypes = @{ $self->{def}{argTypes} };
	my $n        = 0;
	foreach my $x (@{ $self->{params} }) {
		my $atype = shift(@argTypes);
		$n++;
		$self->Error("The %s argument for '%s' should be of type %s", NameForNumber($n), $name, $atype->{name})
