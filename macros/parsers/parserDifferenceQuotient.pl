...
standard C<S<< ->cmp >>> routine will work for this.  The difference quotient
...
Use C<DifferenceQuotient> (formula) to create a difference equation
...
C<DifferenceQuotient()>.  You could use a variable like h instead of
