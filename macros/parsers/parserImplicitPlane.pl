...
This is a C<Parser> class that implements implicit planes as
a subclass of the C<Formula> class.  The standard C<S<< ->cmp >>> routine
...
Use C<ImplicitPlane(point,vector)>, C<ImplicitPlane(point,number)> or
C<ImplicitPlane(formula)> to create an C<ImplicitPlane> object.
...
The number of variables in the C<Context> determines the dimension of
