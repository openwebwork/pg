...
sub check_eigenvector {
	...
	$ans_eval->install_post_filter(sub {
		my $rh_ans = shift;
...
sub level_curve_check {
	...
	$answer_evaluator->ans_hash(
		correct_ans    => $rh_correct_ans->{correct_ans},         # used for answer only
	...
	$answer_evaluator->install_post_filter(
		sub { my $ans_hash = shift; $ans_hash->{correct_ans} = $correctEqn; $ans_hash; });
