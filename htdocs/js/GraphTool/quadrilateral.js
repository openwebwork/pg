		...
		Quadrilateral(gt) {
			return class extends gt.GraphObject {
				...
				constructor(point1, point2, point3, point4, solid) {
					...
					super(
						gt.graphObjectTypes.triangle.createPolygon(
				...
				fillCmp(point) {
					...
					for (let i = 0, j = this.definingPts.length - 1; i < this.definingPts.length; j = i++) {
						if (
							...
							gt.areColinear(point.slice(1), this.definingPts[i], this.definingPts[j])
						) {
							return 0;
						}
					}
				...
				static restore(string) {
					...
					const point4 = this.createPoint(parseFloat(points[3][0]), parseFloat(points[3][1]), [
						point1,
						point2,
						point3
					]);
				...
				// JSXGraph.  Note that this ensures that the graphed object is a quadrilateral, and does not
				// degenerate.
				static adjustDragPosition(e, point, groupedPoints) {
					...
					} else if (groupedPoints.length == 3 && gt.arePairwiseColinear([x, y], ...groupedPoints)) {
						// Adjust the position of the point if it is on any of the
						// lines passing through the pairs of grouped points.
						if (e.type === 'pointermove') {
							const coords = gt.getMouseCoords(e);

							// Of the points to the upper left of, above, upper right of, left of, right of, lower
							// left of, below, and lower right of the current point, find those that are on the
							// board and not on the line between the two grouped points.
							const points = [
								...
								[(x, y - gt.snapSizeY)],
								[x + gt.snapSizeX, y - gt.snapSizeY]
							].filter((p) => gt.boardHasPoint(...p) && !gt.arePairwiseColinear(p, ...groupedPoints));
							...
							x += xDir * gt.snapSizeX;
							y += yDir * gt.snapSizeY;
							if (gt.arePairwiseColinear([x, y], ...groupedPoints)) {
								x += xDir * gt.snapSizeX;
								y += yDir * gt.snapSizeY;
							}
						}
		...
		QuadrilateralTool(gt) {
			return class extends gt.GenericTool {
				...
				phase3(coords) {
					...
					do {
						...
						(!gt.boardHasPoint(newX, newY) ||
							gt.arePairwiseColinear([newX, newY], this.point1, this.point2, this.point3)) &&
						times < 20
					);
				...
				phase4(coords) {
					...
					// If the current coordinates are on the line through the first and second points, or on the
					// line through the first and third points, or on the line through the second and third points,
					// then use the highlight point coordinates instead.
					if (
				...
				updateHighlights(e) {
					...
					if (
						this.point3 &&
						this.hlObjs.hl_quadrilateral &&
						this.hlObjs.hl_quadrilateral.vertices.length < 5
					) {
						...
						this.hlObjs.hl_quadrilateral = gt.graphObjectTypes.triangle.createPolygon(
