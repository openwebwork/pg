	...
	graphTool.quadraticTool = {
		Quadratic(gt) {
			return class extends gt.GraphObject {
				...
				constructor(point1, point2, point3, solid) {
					...
					super(gt.graphObjectTypes.quadratic.createQuadratic(point1, point2, point3, solid, gt.color.curve));
				...
				static createQuadratic(point1, point2, point3, solid, color) {
				...
				// Note that when this method is called, the point has already been moved by JSXGraph.  Note that
				// this ensures that the graphed object is a function, but does not prevent the quadratic from
						...
						if (typeof grouped_points !== 'undefined' && grouped_points.length) {
							...
							for (const paired_point of grouped_points) {
								...
								if (!paired_point.grouped_points) {
									...
									paired_point.on('drag', this.groupedPointDrag);
								}
		...
		QuadraticTool(gt) {
			return class extends gt.GenericTool {
				...
				activationHelpText = 'Plot three points on the quadratic.';
