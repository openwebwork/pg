	...
	graphTool.sineWaveTool = {
		SineWave(gt) {
			return class extends gt.GraphObject {
				...
				constructor(shiftPoint, periodPoint, amplitudePoint, solid) {
					...
					super(
						gt.graphObjectTypes.sineWave.createSineWave(
							shiftPoint,
				...
				static restore(string) {
					...
					const shiftPoint = this.createPoint(
						gt.snapRound(parseFloat(data[1]), gt.snapSizeX),
						gt.snapRound(parseFloat(data[2]), gt.snapSizeY)
					);
					const periodPoint = this.createPoint(
						gt.snapRound(shiftPoint.X() + parseFloat(data[3]), gt.snapSizeX),
						shiftPoint.Y(),
						shiftPoint
					);
					const amplitudePoint = this.createPoint(
						(3 * shiftPoint.X()) / 4 + periodPoint.X() / 4,
						gt.snapRound(shiftPoint.Y() + parseFloat(data[4]), gt.snapSizeY),
						shiftPoint,
						periodPoint
					);
					return new this(shiftPoint, periodPoint, amplitudePoint, /solid/.test(string));
				}

				static createSineWave(point, period, amplitude, solid, color) {
				...
				// Prevent a point from being moved off the board by a drag. If xRestrict is provided, then also
				// prevent the point from being moved into the same vertical line as that point.  If yRestrict is
				// provided, then also prevent the point from being moved into the same horizontal line as that
				// point.  Note that when this method is called, the point has already been moved by JSXGraph.  Note
				// that this ensures that the sine wave does not degenerate into a line or even worse a point.
				static adjustDragPosition(e, point, xRestrict = undefined, yRestrict = undefined) {
				...
				static createPoint(x, y, shiftPoint = undefined, periodPoint = undefined) {
					...
					if (!gt.isStatic) {
						if (shiftPoint) {
							...
							if (!shiftPoint.eventHandlers.drag) shiftPoint.on('drag', this.pointDrag);
							point.on('drag', this.pointDrag);
						}
		...
		SineWaveTool(gt) {
			return class extends gt.GenericTool {
				...
				activationHelpText = 'Move the highlighted point to set the phase shift and vertical translation.';
				useStandardDeactivation = true;
				constructionObjects = ['shiftPoint', 'periodPoint', 'amplitudePoint'];

				constructor(container, iconName, tooltip) {
					super(container, iconName ?? 'sine-wave', tooltip ?? 'Sine Wave Tool: Graph a sine wave.');
				}
				...
				updateHighlights(e) {
					...
					// and that the sine wave is not degenerate.
