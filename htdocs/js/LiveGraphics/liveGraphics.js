...
var LiveGraphics3D = function (container, options) {
	...
	$('<div/>').addClass('sr-only').text('A manipulable 3d graph.').prependTo(container);
	...
	var drawLines = function () {
		...
		lineCoords.forEach(function (line) {
			...
			var shape = $('<shape/>').appendTo(
				$('<transform/>').attr('translation', trans).attr('rotation', rotation).appendTo(linegroup)
			);
			var color = [0, 0, 0];
			var radius = 0.005;
			...
			$('<appearance/>').appendTo(shape).append($('<material/>').attr('diffusecolor', color));
	...
	var drawSurface = function () {
		...
		var indexedfaceset = $('<indexedfaceset/>').attr('coordindex', indexstr).attr('solid', 'false');

		indexedfaceset.append($('<coordinate/>').attr('point', coordstr));

		if (!flatcolor) {
			indexedfaceset.attr('colorindex', colorindstr);
			indexedfaceset.append($('<color/>').attr('color', colorstr));
		}
		...
		if (options.drawMesh) {
			...
			appearance.append($('<material/>').attr('diffusecolor', [0, 0, 0]));

			var indexedlineset = $('<indexedlineset/>').attr('coordindex', indexstr).attr('solid', 'true');

			indexedlineset.append($('<coordinate/>').attr('point', coordstr));
	...
	var drawAxes = function () {
		...
		// into place
		var xgroup = $('<group/>').appendTo(
			$('<transform/>').appendTo(scene).attr('translation', [0, coordMins[1], coordMins[2]])
		);

		var xaxis = $('<shape/>')
			.append($('<appearance/>').append($('<material/>').attr('emissiveColor', 'black')))
			.append($('<Polyline2D/>').attr('lineSegments', coordMins[0] + ' 0 ' + coordMaxs[0] + ' 0'));
		xgroup.append(xaxis);
		...
		var yaxis = $('<shape/>')
			.append($('<appearance/>').append($('<material/>').attr('emissiveColor', 'black')))
			.append($('<Polyline2D/>').attr('lineSegments', coordMins[1] + ' 0 ' + coordMaxs[1] + ' 0'));
		ygroup.append(yaxis);
		...
		var zaxis = $('<shape/>')
			.append($('<appearance/>').append($('<material/>').attr('emissiveColor', 'black')))
			.append($('<Polyline2D/>').attr('lineSegments', coordMins[2] + ' 0 ' + coordMaxs[2] + ' 0'));
	...
	var makeAxisTicks = function (I) {
		...
		for (var i = 0; i < options.numTicks - 1; i++) {
			...
			var tick = $('<shape/>').append(
				$($('<appearance/>').append($('<material/>').attr('diffuseColor', 'black')))
			);
			tick.appendTo($('<transform/>').attr('translation', [coord, 0, 0]));

			tick.append($('<box/>').attr('size', options.tickSize + ' ' + options.tickSize + ' ' + options.tickSize));

			shapes.push(tick.parent());

			// labels have two decimal places and always point towards view
			var ticklabel = $('<shape/>').append(
				$($('<appearance/>').append($('<material/>').attr('diffuseColor', 'black')))
			);
			...
			ticklabel.appendTo(
				$('<billboard/>')
					...
					.appendTo($('<transform/>').attr('translation', [coord, 0.1, 0]))
			);
		...
		axislabel.appendTo(
			$('<billboard/>')
				.attr('axisOfRotation', '0 0 0')
				.appendTo($('<transform/>').attr('translation', [coordMaxs[I], 0.1, 0]))
		);
	...
	var drawLonePoints = function () {
		lonePoints.forEach(function (point) {
			...
			var sphere = $('<shape/>').append(
				$($('<appearance/>').append($('<material/>').attr('diffuseColor', color)))
			);
			sphere.appendTo($('<transform/>').attr('translation', point.coords));

			sphere.append($('<sphere/>').attr('radius', point.radius * 2.25));
	...
	var drawLoneLabels = function () {
		loneLabels.forEach(function (label) {
			...
			text.appendTo(
				$('<billboard/>')
					.attr('axisOfRotation', '0 0 0')
					.appendTo($('<transform/>').attr('translation', label.coords))
			);
			...
			text.append(
				$('<text/>')
					...
					.attr('solid', 'true')
					.append($('<fontstyle/>').attr('size', size).attr('family', 'mono').attr('justify', 'MIDDLE'))
			);
