...
sub cplx_cmp {
	...
	#  Get default options
	#
	assign_option_aliases(\%cplx_params, 'reltol' => 'relTol',);
	set_default_options(
		\%cplx_params,
	...
	#
	my $oldContext = Parser::Context->current(\%main::context, $context);
	my $z;
	if (ref($correctAnswer) =~ /^Complex1?$/) {
		...
		$z = $z->eval;
		$z = new Value::Complex($z) unless Value::class($z) eq 'String';
	}
...
sub compare_cplx {
	...
	$rh_ans->{score} = 1
		if (abs($rh_ans->{correct_ans}->Complex::Re - $inVal->Complex::Re) <= $permitted_error_Re
			&& abs($rh_ans->{correct_ans}->Complex::Im - $inVal->Complex::Im) <= $permitted_error_Im);
...
sub multi_cmp {
	...
	$multi_ans_evaluator->install_evaluator(sub {
		...
		$rh_ans_out->{score}                = 0
			unless @{$ra_answer_evaluators} == @student_answers;    # require the  correct number of answers
...
sub is_a_numeric_polar {
	...
	$num =~ s/^\s*//;    ## remove	initial	spaces
	$num =~ s/\s*$//;    ## remove	trailing spaces
	if ($num =~ /^($number|($number\*)?2.71828182845905\*\*\(($number\*\(i\)|\(i\)\*$number|i|-\(i\))\))$/) {
		$is_a_number = 1;
	}
