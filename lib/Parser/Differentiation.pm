...
sub Parser::BOP::add::D {
	my $self = shift;
	my $x    = shift;
	$self = $self->Item("BOP")->new($self->{equation}, $self->{bop}, $self->{lop}->D($x), $self->{rop}->D($x));
	return $self->reduce;
}
...
sub Parser::BOP::multiply::D {
	...
	$self = $BOP->new(
		$equation, '+',
		$BOP->new($equation, $self->{bop}, $self->{lop}->D($x),           $self->{rop}->copy($equation)),
		$BOP->new($equation, $self->{bop}, $self->{lop}->copy($equation), $self->{rop}->D($x))
	);
	return $self->reduce;
}
...
sub Parser::BOP::divide::D {
	...
	$self = $BOP->new(
		...
		$BOP->new(
			$equation, '-',
			$BOP->new($equation, '*', $self->{lop}->D($x),           $self->{rop}->copy($equation)),
			$BOP->new($equation, '*', $self->{lop}->copy($equation), $self->{rop}->D($x))
		),
...
sub Parser::Function::trig::D_cos {
	...
	my $x        = shift;
	my $equation = $self->{equation};
	return $self->Item("UOP")->new($equation, 'u-', $self->Item("Function")->new($equation, 'sin', [$x]));
}
...
sub Parser::Function::trig::D_sec {
	...
	my $equation = $self->{equation};
	my $FN       = $self->Item("Function");
	return $self->Item("BOP")->new($equation, '*', $FN->new($equation, 'sec', [$x]), $FN->new($equation, 'tan', [$x]));
}
...
sub Parser::Function::trig::D_csc {
	...
	my $equation = $self->{equation};
	my $FN       = $self->Item("Function");
	return $self->Item("UOP")->new($equation, 'u-',
		$self->Item("BOP")->new($equation, '*', $FN->new($equation, 'csc', [$x]), $FN->new($equation, 'cot', [$x]))
	);
}
...
sub Parser::Function::trig::D_atan {
	...
	my $BOP      = $self->Item("BOP");
	my $NUM      = $self->Item("Number");
	return $BOP->new($equation, '/', $NUM->new($equation, 1),
		$BOP->new($equation, '+', $NUM->new($equation, 1), $BOP->new($equation, '^', $x, $NUM->new($equation, 2))));
}
...
sub Parser::Function::hyperbolic::D_sech {
	...
	return $self->Item("UOP")->new($equation, 'u-',
		$self->Item("BOP")
			->new($equation, '*', $FN->new($equation, 'sech', [$x]), $FN->new($equation, 'tanh', [$x])));
}
...
sub Parser::Function::hyperbolic::D_csch {
	...
	return $self->Item("UOP")->new($equation, 'u-',
		$self->Item("BOP")
			->new($equation, '*', $FN->new($equation, 'csch', [$x]), $FN->new($equation, 'coth', [$x])));
}
...
sub Parser::Function::numeric::D_log {
	...
	if   ($base10) { return $self->D_log10(@_) }
	else           { return $self->D_ln(@_) }
}
...
sub Parser::Function::numeric::D_log10 {
	...
	return $BOP->new(
		$equation, '/',
		$NUM->new($equation, 1),
		$BOP->new($equation, '*', $NUM->new($equation, CORE::log(10)), $x)
	);
}
...
sub Parser::Function::numeric::D_sqrt {
	...
	return $BOP->new(
		$equation, '/',
		$NUM->new($equation, 1),
		$BOP->new($equation, '*', $NUM->new($equation, 2), $self->copy)
	);
}
...
sub Parser::List::AbsoluteValue::D {
	my $self     = shift;
	my $x        = $self->{coords}[0]->copy;
	my $equation = $self->{equation};
	my $BOP      = $self->Item("BOP");
	return $BOP->new($equation, "*", $BOP->new($equation, '/', $x, $self->copy), $x->D(shift),);
}
