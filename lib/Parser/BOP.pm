...
sub new {
	...
	if (!$def->{isComma}) {
		$lop =
			$self->Item("List", $context)
			->new($equation, [ $lop->makeList ], $lop->{isConstant}, $context->{parens}{start})
			if ($lop->type eq 'Comma');
		$rop =
			$self->Item("List", $context)
			->new($equation, [ $rop->makeList ], $rop->{isConstant}, $context->{parens}{start})
			if ($rop->type eq 'Comma');
	}
	my $BOP = bless {
		bop      => $bop,
		lop      => $lop,
...
sub checkStrings {
	my $self = shift;
	return 0 if $self->context->flag("allowBadOperands");
	my $ltype = $self->{lop}->typeRef;
	my $rtype = $self->{rop}->typeRef;
	my $name  = $self->{def}{string} || $self->{bop};
	if ($ltype->{name} eq 'String') {
		$self->Error("Operands of '%s' can't be %s", $name, ($self->{lop}{isInfinite} ? 'infinities' : 'words'));
		return 1;
	}
	if ($rtype->{name} eq 'String') {
		$self->Error("Operands of '%s' can't be %s", $name, ($self->{rop}{isInfinite} ? 'infinities' : 'words'));
		return 1;
	}
	return 0;
}
...
sub checkNumbers {
	my $self = shift;
	return 0
		if !($self->{lop}->isNumber && $self->{rop}->isNumber)
...
sub checkMatrixSize {
		...
		&& $rtype->{entryType}{name} eq 'Number')
	{
		...
		if ($lc == $rr) {
			...
			$self->Error("Matrices of dimensions %dx%d and %dx%d can't be multiplied", $lr, $lc, $rr, $rc);
		}
	} else {
...
sub matchError {
	...
	my ($ltype, $rtype) = @_;
	my ($op,    $ref)   = ($self->{bop});
	if   ($ltype->{name} eq $rtype->{name}) { $self->Error("Operands for '%s' must be of the same length", $op) }
...
sub TeX {
	...
	my $addparens   = defined($precedence)
		&& (
			($showparens eq 'all' && $extraParens > 1)
			|| $precedence > $bop->{precedence}
			|| ($precedence == $bop->{precedence}
				&& ($bop->{associativity} eq 'right' || $showparens eq 'same'))
		);
...
sub perl {
	...
	my $perl;
	if ($bop->{isCommand}) {
		$perl = ($bop->{perl} || ref($self) . '->call') . '(' . $self->{lop}->perl . ',' . $self->{rop}->perl . ')';
	} else {
		$perl = $self->{lop}->perl(1) . " " . ($bop->{perl} || $bop->{string}) . " " . $self->{rop}->perl(2);
	}
	$perl = '(' . $perl . ')' if $parens;
	return $perl;
}
