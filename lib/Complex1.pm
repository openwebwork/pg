...
sub cplxe {
	my ($rho, $theta) = @_;
	return $package->emake(defined $rho ? $rho : 0, defined $theta ? $theta : 0);
}
...
sub cartesian {
	$_[0]->{c_dirty} ? $_[0]->update_cartesian : $_[0]->{'cartesian'};
}

sub polar {
	$_[0]->{p_dirty} ? $_[0]->update_polar : $_[0]->{'polar'};
}
...
sub minus {
	...
	return ($inverted) ? (ref $z1)->make($re2 - $re1, $im2 - $im1) : (ref $z1)->make($re1 - $re2, $im1 - $im2);
...
sub power {
	...
	return $z1->{c_dirty} == 0
		&& (not ref $z2 or $z2->{c_dirty} == 0) ? cplx(@{ $w->cartesian }) : $w;
}
...
sub log10 {
	return Complex1::log($_[0]) * uplog10;
}
...
sub cos {
	...
	my $ey   = CORE::exp($y);
	my $ey_1 = 1 / $ey;
	return (ref $z)->make(CORE::cos($x) * ($ey + $ey_1) / 2, CORE::sin($x) * ($ey_1 - $ey) / 2);
}
...
sub sin {
	...
	my $ey   = CORE::exp($y);
	my $ey_1 = 1 / $ey;
	return (ref $z)->make(CORE::sin($x) * ($ey + $ey_1) / 2, CORE::cos($x) * ($ey - $ey_1) / 2);
}
...
sub cosh {
	...
	$ex = CORE::exp($x);
	my $ex_1 = 1 / $ex;
	return (ref $z)->make(CORE::cos($y) * ($ex + $ex_1) / 2, CORE::sin($y) * ($ex - $ex_1) / 2);
}
...
sub stringify_polar {
	...
	if (defined $theta) {
		...
		$theta = int($theta + ($theta < 0 ? -1 : 1) * $eps)
			if ($theta ne 'pi'
				and int(CORE::abs($theta)) != int(CORE::abs($theta) + $eps));
		return "\[$r,$theta\]";
	}
...
By calling the routine C<Complex1::display_format> and supplying either
...
messages like the following
    ...
    Complex1::make: Cannot take real part of ...
    Complex1::make: Cannot take real part of ...
    Complex1::emake: Cannot take rho of ...
    Complex1::emake: Cannot take theta of ...
