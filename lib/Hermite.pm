...
sub initialize {
	...
	%{ $self->{rh_critical_points} } = critical_points($self->{ra_x}, $self->{ra_y}, $self->{ra_yp}, $self->{rf_f},);
	#     # define the inflection_points
	%{ $self->{rh_inflection_points} } =
		inflection_points($self->{ra_x}, $self->{ra_y}, $self->{ra_yp}, $self->{rf_f},);
...
sub critical_points {
	...
	foreach my $i (0 .. $last_index) {
		internal_critical_points(
			$ra_x->[$i],       $ra_y->[$i],        $ra_yp->[$i],      $ra_x->[ $i + 1 ],
			$ra_y->[ $i + 1 ], $ra_yp->[ $i + 1 ], \%critical_points, $rf_hermite_fun
		);
	}
	%critical_points;
}
...
sub inflection_points {
	...
	my $last_index        = @$ra_x - 2;
	foreach my $i (0 .. $last_index) {
		internal_inflection_points(
			$ra_x->[$i], $ra_y->[$i], $ra_yp->[$i],
			$ra_x->[ $i + 1 ],
			$ra_y->[ $i + 1 ],
			$ra_yp->[ $i + 1 ],
...
sub internal_critical_points {
	...
	my @a = ($l, $lp, -$lp / 2 + (3 * (-$lp - 2 * ($l - $r) - $rp)) / 2 + $rp / 2, $lp + 2 * ($l - $r) + $rp);
