...
sub _dot {
	...
	return bop('.', @_)
		if ($l->type eq 'Vector' || $l->{isVector})
		&& Value::isValue($r)
		&& ($r->type eq 'Vector' || $r->{isVector});
	$l->SUPER::_dot($r, $flag);
}
...
sub noinherit {
	my $self = shift;
	(
		$self->SUPER::noinherit(@_),
		"test_values", "test_adapt", "tree", "string", "variables",
...
sub createAdaptedValues {
	...
	foreach my $p (@{$points}) {
		...
		if (!defined($v)) {
			return unless $showError;
			Value::Error(
				"Can't evaluate formula on test point (%s) with parameters (%s)",
				join(',', @{$p}),
				join(',', @adapt)
			);
		}
...
sub createRandomPoints {
	my $self    = shift;
	my $context = $self->context;
	my ($num_points, $include, $noErrors) = @_;
	my $cacheResults = !defined($num_points);
	...
	while (scalar(@{$points}) < $num_points + $num_undef && $k < 10) {
		...
		foreach my $limit (@limits) {
			@p = ();
			foreach my $I (@{$limit}) { push @p, $context->Package("Real")->make($context, $self->$getRandom(@{$I})) }
...
sub getVariableLimits {
	...
	my @limits;
	my $default;
	$default = $userlimits->[0][0] if defined($userlimits->[0]);
...
sub AdaptParameters {
	...
	my $D;
	return 0 if $d == 0;
	return 0 unless $l->usesOneOf(@params);
	...
	foreach my $attempt (1 .. 3) {
		...
		#
		my $M = MatrixReal1->new($d, $d);
		$M->[0] = \@A;
		my $B = MatrixReal1->new($d, 1);
		$B->[0] = \@b;
		($M, $B) = $M->normalize($B);
		$M = $M->decompose_LR;
		if (CORE::abs($M->det_LR) > 1E-6) {
			if (($D, $B, $M) = $M->solve_LR($B)) {
				if ($D == 0) {
					...
					my @a;
					my $i   = 0;
					my $max = $l->getFlag('max_adapt', 1E8);
					foreach my $row (@{ $B->[0] }) {
						if (CORE::abs($row->[0]) > $max) {
							$max = Value::makeValue($max);
							$row->[0] = Value::makeValue($row->[0]);
							$l->Error([
								"Constant of integration is too large: %s\n(maximum allowed is %s)",
								$row->[0]->string, $max->string
							])
								if $params[$i] eq 'C0'
								or $params[$i] eq 'n00';
							$l->Error([
								"Adaptive constant is too large: %s = %s\n(maximum allowed is %s)",
