...
$tokenregexp =
	"(?:($binoper3)|($binoper2)|($binoper1)|($openparen)|"
...
sub parse {
	...
	while ($currenttok) {
		...
		/binop1/ && do {
			...
			if (defined(${$currentref})) {
				...
				${$currentref}->[$index] = [ 'binop1', $currenttok->[1], ${$currentref}->[$index], undef ];
		...
		/binop2/ && do {
			if (defined(${$currentref})) {
				...
				${$currentref}->[$index] = [ 'binop2', $currenttok->[1], ${$currentref}->[$index], undef ];
				push @backtrace, $currentref;
				push @backtrace, \${$currentref}->[$index];
				$currentref = \${$currentref}->[$index]->[3];
			} else {
				...
				my ($mark) = pop @{ $self->{posarray} };
				my $position = 1 + $mark->[0];
				return $self->error("Didn't expect " . $currenttok->[1] . " at position $position", $mark);
			}
		};
		/binop3/ && do {
			if (defined(${$currentref})) {
				...
				${$currentref}->[$index] = [ 'binop3', $currenttok->[1], ${$currentref}->[$index], undef ];
				push @backtrace, $currentref;
				push @backtrace, \${$currentref}->[$index];
				$currentref = \${$currentref}->[$index]->[3];
			} else {
		...
		/closep/ && do {
			if (defined(${$currentref})) {
				...
				if ($a eq 'top') {
					my ($mark) = pop @{ $self->{posarray} };
					my $position = 1 + $mark->[0];
					return $self->error("Unmatched close " . $currenttok->[1] . " at position $position", $mark);
				} elsif ($close{ ${$currentref}->[1] } ne $currenttok->[1]) {
					my ($mark) = pop @{ $self->{posarray} };
					my $position = 1 + $mark->[0];
					return $self->error("Mismatched parens at position $position", ${$currentref}->[2], $mark);
				} else {
					${$currentref}->[0] = 'closep';
				...
				my ($mark) = pop @{ $self->{posarray} };
				my $position = 1 + $mark->[0];
				return $self->error("Premature close " . $currenttok->[1] . " at position $position", $mark);
			}
		};
	...
	if ($self->{string} =~ m/\G\s*$/g) {
		if (!defined(${$currentref})) {
			$self->{string} .= " ";
			return $self->error(
				"I was expecting more at the end of the line",
			...
			while (@backtrace) {
				...
				if (${$currentref}->[0] eq 'openp') {
					...
					my $position = 1 + $mark->[0];
					return $self->error("Unclosed parentheses beginning at position $position", $mark);
				}
			}
...
sub error {
	...
	while (@markers) {
		...
		$htmledstring .=
			encode_entities(substr($str, $lastpos, $pos1 - $lastpos))
			. '<b class="parsehilight">'
			. encode_entities(substr($str, $pos1, $pos2 - $pos1)) . '</b>';
		$lastpos = $pos2;
	}
...
sub normalize {
	...
	if (/closep/) {
		...
		$ret = $class->new($type, $args[0], $args[1]->normalize($degree), $args[2]->normalize($degree));
	} elsif (/func1/) {
		$args[0] =~ s/^arc/a/;
		$ret = $class->new($type, $args[0], $args[1]->normalize($degree));
	}
	...
	if (/binop1/ && ($args[2]->[0] =~ 'unop1')) {
		...
		$ret = $class->new('unop1', '-', $class->new($type, $args[0], $args[1]->[2], $args[2])->normalize($degree));
	} elsif (/binop[12]/
		&& ($args[2]->[0] eq $type)
		&& ($args[0] =~ /[+*]/))
	{
		...
		$ret =
			$class->new($type, $args[2]->[1], $class->new($type, $args[0], $args[1], $args[2]->[2])->normalize($degree),
				$args[2]->[3]);
...
sub tostring {
	...
	/binop1/ && do {
		...
		return ($args[1]->tostring() . $args[0] . $p1 . $args[2]->tostring() . $p2);
	};
	/unop1/ && do {
		my ($p1, $p2) = ('', '');
	...
	/binop2/ && do {
		...
		return ($p1 . $args[1]->tostring() . $p2 . $args[0] . $p3 . $args[2]->tostring() . $p4);
	};
	/binop3/ && do {
		my ($p1, $p2, $p3, $p4) = ('', '', '', '');
...
sub tolatex {
	...
	/binop1/ && do {
		...
		return ($args[1]->tolatex() . $args[0] . $p1 . $args[2]->tolatex() . $p2);
	};
	/unop1/ && do {
		my ($p1, $p2) = ('', '');
	...
	/binop3/ && do {
		...
		return ($p1 . $args[1]->tolatex() . $p2 . "^{" . $p3 . $args[2]->tolatex() . $p4 . "}");
	};
	/func1/ && do {
		my ($p1, $p2);
	...
	/closep/ && do {
		...
		return ('\left' . $backslash . $args[0] . $args[1]->tolatex() . '\right' . $backslash . $close{ $args[0] });
	};
}
...
sub tostring {
	...
	if (($type eq qq(binop2)) && ($args[0] eq qq(implicit))) {
		...
		return ($p1 . $args[1]->tostring() . $p2 . '*' . $p3 . $args[2]->tostring() . $p4);
	} else {
