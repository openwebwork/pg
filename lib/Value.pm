...
sub isHash {
	my $self = shift;
	return
		defined($self) && (ref($self) || blessedType($self)) && (ref($self) eq 'HASH' || blessedType($self) eq 'HASH');
...
sub canBeInUnion {
	...
	my $open = $self->{open};
	$open = $def->{open} || $def->{nestedOpen} unless defined $open;
	my $close = $self->{close};
	$close = $def->{close} || $def->{nestedClose} unless defined $close;
	return
		$self->length == 2
		&& $self->typeRef->{entryType}{name} eq 'Number'
		&& $open  =~ m/^[\(\[]$/
		&& $close =~ m/^[\)\]]$/;
}
...
sub formula {
	...
	my $close   = $list->{'close'};
	my $paren   = $open;
	$paren = 'list' if $self->classMatch('List');
	my $formula = $self->Package("Formula")->blank($context);
	my @coords  = Value::toFormula($formula, @{$values});
	$formula->{tree} = $formula->Item("List")
		->new($formula, [@coords], 0, $formula->{context}{parens}{$paren}, $coords[0]->typeRef, $open, $close);
...
sub promotePrecedence {
	...
	my $sprec = $self->precedence;
	my $oprec = $other->precedence;
...
sub _dot {
	...
	if   (Value->context->flag('StringifyAsTeX')) { $l = $l->TeX }
	else                                          { $l = $l->pdot }
	return ($flag) ? ($r . $l) : ($l . $r);
}
#
...
sub TeX {
	...
	$open  =~ s/([{}])/\\$1/g;
	$close =~ s/([{}])/\\$1/g;
	$open  = '\left' . $open   if $open;
	$close = '\right' . $close if $close;
	my @coords = ();
	my $str    = $context->{strings};

	foreach my $x (@{ $self->data }) {
		if (Value::isValue($x)) {
			$x->{format} = $self->{format} if defined $self->{format};
			push(@coords, $x->TeX($equation));
		} elsif (defined($str->{$x}) && $str->{$x}{TeX}) {
			push(@coords, $str->{$x}{TeX});
		} else {
			push(@coords, $x);
		}
	}
...
sub perl {
	...
	my $parens = shift // 0;
	my $matrix = shift;
	my $mtype  = $self->classMatch('Matrix');
	$mtype = -1 if $mtype & !$matrix;
	my $perl;
	my @p = ();
	foreach my $x (@{ $self->data }) {
		if (Value::isValue($x)) { push(@p, $x->perl(0, $mtype)) }
		else                    { push(@p, $x) }
	}
...
sub traceback {
	my $frame = shift;
	$frame = 2 unless defined($frame);
	my $trace = '';
	while (my ($pkg, $file, $line, $subname) = caller($frame++)) { $trace .= " in $subname at line $line of $file\n" }
	return $trace;
}
