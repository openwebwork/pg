...
use Renderer::IO qw(fileFromPath);
use utf8;
use v5.12;
...
Renderer::Translator - Evaluate PG code and evaluate answers safely

=head1 SYNPOSIS

    my $pt = new Renderer::Translator;      # create a translator;
...
BEGIN {
	...
	if (exists($ENV{MOJO_MODE})) {
		...
		my $filePath             = $PG::Constants::PG_DIRECTORY . "/macros/PG.pl";
		...
		$PG::Translator::safeCache = $safeCache;
	}
...
In addition the environment hash C<%envir> environment variables for each problem
template.
...
# functions shared from Renderer::Translator
...
# add names from Renderer::IO and Renderer::IO::*
my %IO_shared_subroutine_hash = %Renderer::IO::SHARE;
...
sub PG_errorMessage {
	my $return  = shift;
	my $frame   = 2;                # return can be 'message' or 'traceback'
	my $message = join("\n", @_);
	$message =~ s/\.?\s+$//;
...
=cut
	...
	if ($self->{errors} && $self->{envir}->{VIEW_PROBLEM_DEBUGGING_INFO} <= $permissionLevel) {
		...
		warn "ERRORS in rendering problem\n";
...
sub process_answers {
	...
	foreach my $ans_name (keys %{ $PG->{PG_ANSWERS_HASH} }) {
		...
		my $answergrp =
			$PG->{PG_ANSWERS_HASH}->{$ans_name};    #this has all answer evaluators AND answer blanks (just to be sure )
