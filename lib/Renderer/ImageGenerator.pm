...
package Renderer::ImageGenerator;

=head1 NAME

Renderer::ImageGenerator - create an object for holding bits of math for
...
use Renderer::Utils::EquationCache;

# can't use Renderer::Utils from here, so we define the needed functions here
#use Renderer::Utils qw/readFile readDirectory makeTempDirectory removeTempDirectory/;
...
our $TexPreamble = "" unless defined $Renderer::ImageGenerator::TexPreamble;
...
sub new {
	...
	if ($self->{useCache}) {
		...
		$self->{basename}      = "";
		$self->{equationCache} = Renderer::Utils::EquationCache->new(cacheDB => $self->{cacheDB});
	}
