...
package Renderer::FormatRenderedProblem;

use Renderer::Utils::AttemptsTable;
use Renderer::Utils qw(wwRound decode_utf8_base64);
use XML::Simple qw(XMLout);
use Renderer::Utils::LanguageAndDirection;
use JSON;
...
sub formatRenderedProblem {
	...
	# Create a PG environment
	my $pg_env = PGEnvironment->new(course_name => $courseID);

	my $mt = Renderer::Localize::getLangHandle($self->{inputs_ref}{language} // 'en');

	my $forbidGradePassback = 1;                       # Default is to forbid, due to the security issue

	# Note: not sure where html2xmlAllowGradePassback this field of $ce would come from
...
# It is strongly recommended that you clarify the security risks of enabling the current version of this feature before using it.
	...
	# PG shouldn't need any theme information.
	...
	my $webwork_htdocs_url = '/pg_files';
	...
	if (ref($ce->{pg}{specialPGEnvironmentVars}{extra_css_files}) eq "ARRAY") {
		push(@cssFiles, { file => $_, external => 0 }) for @{ $pg_env->{env}->{extra_css_files} };
	}
	...
	if ($renderErrorOccurred) {
		...
		my $tbl = Renderer::Utils::AttemptsTable->new(
			$rh_result->{answers} // {},
			...
			maketext            => Renderer::Localize::getLoc($formLanguage),
	...
	if ($rh_result->{pg_warnings}) {
		$warnings .=
			qq{<div style="background-color:pink">PG WARNINGS<br>}
			. decode_utf8_base64($rh_result->{pg_warnings})
			. "</div>";
	}
	if ($rh_result->{translator_warnings}) {
		$warnings .=
			qq{<div style="background-color:pink"><p>TRANSLATOR WARNINGS</p><p>}
	...
	my $footer =
		$showFooter eq "0"
		? ''
		: "<div id='footer'>WeBWorK &copy; 2000-2021 | host: $SITE_URL | course: $courseID | format: $self->{inputs_ref}{outputformat}</div>";
	...
	say $format_name;
	say $pg_env->{directories}->{problem_templates};
	say "$pg_env->{directories}->{problem_templates}/${format_name}_format.pl";
	# Find and execute the appropriate template in the WebworkClient folder.
	my $template = do("$pg_env->{directories}->{problem_templates}/${format_name}_format.pl");
