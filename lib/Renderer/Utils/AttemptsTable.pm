...
package Renderer::Utils::AttemptsTable;
use base qw(Class::Accessor);

use strict;
use warnings;
use Scalar::Util 'blessed';
use Renderer::Utils 'wwRound';
use CGI;
...
sub _init {
	...
	if ($self->displayMode eq 'images') {
		if (blessed($options{imgGen})) {
			...
			my $imgGen = Renderer::ImageGenerator->new(
...
sub formatAnswerRow {
	...
	my $row = join(
		...
		($self->showAttemptAnswers)
		? CGI::td({ dir => "auto" }, $self->nbsp($answerString))
...
sub previewCorrectAnswer {
	...
	return $answerResult->{correct_ans}
		unless defined $tex and $tex =~ /\S/;    # some answers don't have latex strings defined
												 # return "" unless defined $tex and $tex ne "";
...
sub createSummary {
	...
	$summary = CGI::div({ role => "alert", class => "attemptResultsSummary" }, $summary);
