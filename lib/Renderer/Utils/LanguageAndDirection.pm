...
package Renderer::Utils::LanguageAndDirection;
use base qw(Exporter);

=head1 NAME

Renderer::Utils::LanguageAndDirection - utilities to determine the language and text
...
sub get_problem_lang_and_dir {
	...
	if (
		($dir eq "rtl") &&       # Possible hack for RTL direction courses and OPL problems
		($reqDir eq "rtl")
		&& !defined($pg_flags->{textdirection})
		&&                       # problem does not set the language or
		!defined($pg_flags->{language})
		)
	{                            # the text direction
