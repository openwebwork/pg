...
# 		"begin");
	...
	if ($translationOptions->{displayMode} eq "images" || $translationOptions->{displayMode} eq "opaque_image") {
		...
		$image_generator = WeBWorK::PG::ImageGenerator->new(
			tempDir         => $pg_env->{renderer}->{temp_dir}, # temp directory in htdocs/tmp
			latex           => $pg_env->{environment}->{externalPrograms}->{latex},
			dvipng          => $pg_env->{environment}->{externalPrograms}->{dvipng},
			useCache        => 1,
			cacheDir        => $pg_env->{renderer}->{equation_cache},
			cacheURL        => $site_prefix . $pg_env->{renderer}->{equation_cache},
			cacheDB         => $pg_env->{environment}->{equation_cache_db},
	...
	# This doesn't seem like a good place for this.  Can we move this to WeBWorK?
	...
	foreach (qw(PG.pl )) {    # dangerousMacros.pl IO.pl
		my $macroPath = $pg_env->{environment}->{pg_macros} . "/$_";
	...
	if (ref($translationOptions->{r_source})) {
		...
		$sourceFilePath = $pg_env->{environment}->{course_directory} . "/templates/" . $sourceFilePath unless ($sourceFilePath =~ /^\//);
		#now grab the source
...
EOF
	...
	if ($translationOptions->{processAnswers}) {
		...
		my $grader = $translator->rh_flags->{PROBLEM_GRADER_TO_USE} || $pg_env->{renderer}->{grader};
	...
	if ($image_generator) {
		my $sourceFile = $pg_env->{environment}->{course_directory} . "/templates/" . $problem->source_file;
